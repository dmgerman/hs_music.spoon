...
- =changeMusicAppVolume(level)= - Set volume to a specific level (0-100 percent); returns new volume or nil
- =getMusicAppVolume()= - Get current volume as percentage (0-100)
- =changeVolume(delta)= - Change volume by a given amount (can be negative); returns new volume or nil
